...
class DecadaPythonClient:
    ...
    #Created additional postEvent function to post events to DECADA - 14/10/20
    def postEvent(self, eventDict):
        eventPostRequestBuilder = EventPostRequest.builder() \
            .set_product_key(self.__productKey) \
            .set_device_key(self.__deviceKey)

        for key, value in eventDict.items():
            # event identifier must be same as the one set in DECADA
            eventPostRequestBuilder.set_event_identifier("CameraEvents")
            eventPostRequestBuilder = eventPostRequestBuilder.add_value(
                key, value)

        eventPostRequest = eventPostRequestBuilder.build()
        eventPostResponse = self.__mqttClient.publish(
            eventPostRequest)
        if eventPostResponse:
            print("EventPostResponse: {}".format(
                eventPostResponse.get_code()))


if __name__ == '__main__':

    client = DecadaPythonClient(os.getcwd(), "/config.yaml")
    client.connect()

    while True:
        for i in range(100):
            #time.sleep(1)
            y = {'temperature': 34.5 + i, 'humidity': 70+i}
            #client.postMeasurePoints({'temperature': 34.5+i, 'humidity': 70+i})
            client.postMeasurePoints(y)
